...
// === Context Gateway ========================================================
// Isolated block — minimal STATE dependency (only ctxCache). Future split to
// context-gateway.js: extract this block, add <script src="/context-gateway.js">
...
document.getElementById('ctx-detect-btn')?.addEventListener('click', async () => {
  ...
  btnLoading(btn, true);
  try {
    // Detect just refreshes the overview for now
...
async function loadCtxDetail(type, name) {
  ...
  try {
    ...
    // Store mtime for optimistic concurrency
    ...
    // Diff button (switches to diff tab)
...
document.querySelectorAll('.ctx-import-btn').forEach(btn => {
  btn.addEventListener('click', async () => {
    ...
    try {
      ...
      const data = await r.json();
      // Show result panel instead of just toast
      const statusEl = qs(`ctx-${type}-status`);
